# فيه جهازين و التأخير و الطاقة معدلين
import numpy as np
import torch
...
class MECServer:
    ...
    def simulate_training_round(self, selected_devices, epochs=10, episode=0):
        ...
        mean_iteration_energy = np.mean(total_energy) if total_energy else 0
        self.energy_history.append(total_energy)
        self.bandwidth_history.append(total_bandwidth)
        max_latency = np.mean(delays) if delays else 0 
        return max_latency, total_energy, total_bandwidth, self.energy_history, mean_iteration_energy
...
class DeviceSelectionEnv:
    ...
    def step(self, action, episode=0):
        ...
        max_latency, total_energy_consumed, total_bandwidth, e, mean_iteration_energy = self.mec_server.simulate_training_round(selected_indices, episode=episode)
...
class DDQNAgent:
    ...
    def select_action(self, state):
        ...
        if random.random() < self.epsilon:
            num_selected = random.randint(1, n//2)  # اختيار عشوائي بين 1 ونصف الأجهزة
            ...
            with torch.no_grad():
                ...
                with open("scores_output_fixed_en_lat_oct10.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    for episode in range(NUM_EPISODES):
        ...
        while accuracy < DESIRED_ACCURACY:
            ...
            next_state, reward, max_latency, mean_iteration_energy = env.step(action, episode)
        ...
        episode_energy_history.append(np.mean(episode_energy_list) if episode_energy_list else 0)
    ...
    plt.plot(episode_energy_history, label="Average energy per episode")
    plt.xlabel("Episode")
