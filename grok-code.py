...
class EdgeDevice:
    ...
    def train_local_model(self, epochs, energy_rate=1):
        ...
        print(f"Device {self.id}: Trained, Energy used: {B_k:.2f} pJ, Bandwidth: {self.effective_bandwidth:.2f} Mbps")
...
class MECServer:
    ...
    def simulate_training_round(self, selected_devices, epochs=3):
        ...
        return max(delays) if delays else 0, total_energy, len(local_weights), total_bandwidth
...
class DeviceSelectionEnv:
    ...
    def step(self, action):
        ...
        max_latency, total_energy_consumed, num_valid_devices, total_bandwidth = self.mec_server.simulate_training_round(selected_indices)
    ...
    def reset(self):
        for device in self.mec_server.devices:
            # يجب علينا أن نتناقش في حجم الطاقة الأولية لكل جهاز هنا
            device.energy = 5000 + (device.cpu_freq * 1000)
        self.e_max_per_device = np.array([5000 + (device.cpu_freq * 1000) for device in mec_server.devices])
        self.e_max_total = np.sum(self.e_max_per_device)
