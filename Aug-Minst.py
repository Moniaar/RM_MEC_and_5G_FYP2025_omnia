...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('5ep_10ta_19_7_4.txt', mode='a'),
...
class GlobalModel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, padding=1)  # Changed input channels to 1 for MNIST
        self.conv2 = nn.Conv2d(32, 32, kernel_size=3, padding=1)
        self.pool1 = nn.MaxPool2d(2, 2)
        self.conv3 = nn.Conv2d(32, 64, kernel_size=3, padding=1)
        self.conv4 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.pool2 = nn.MaxPool2d(2, 2)
        self.fc1 = nn.Linear(64 * 7 * 7, 128)  # Adjusted for 28x28 MNIST images: (28/2)/2 = 7
...
if __name__ == "__main__":
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))  # Changed to single channel for MNIST
    ])
    ...
    for episode in range(NUM_EPISODES):
        ...
        episode_accuracy_history.append(iteration_accuracies)
        mean_ep_accuracy = np.mean(iteration_accuracies) if iteration_accuracies else 0
    ...
    plt.subplot(1, 4, 1)
    plt.plot(mean_ep_accuracy, label="Mean Accuracy per episode")
    plt.xlabel("Episode")
    plt.ylabel("Mean Accuracy")
    plt.title("Mean Accuracy per episode")
    plt.legend()
    ...
    plt.subplot(1, 4, 4)
    plt.plot(episode_accuracy_history, label="Accuracy per iteration")
